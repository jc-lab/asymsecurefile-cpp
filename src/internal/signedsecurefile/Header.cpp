...
namespace asymsecurefile {
    namespace internal {
        namespace signedsecurefile {
            ...
            unsigned char Header::DATA_IV[] = {0x92, 0xe5, 0x26, 0x21, 0x1e, 0xda, 0xca, 0x0f, 0x89, 0x5f, 0x2b, 0x74,
                                               0xc1, 0xc4, 0xb4, 0xb9};

            int Header::COMMON_HEADER_SIZE = 32;
            int Header::SECURE_HEADER_SIZE = 84;
            int Header::VERSION = 2;
            unsigned char Header::SIGNATURE[] = {0x0a, 0x9b, 0xd8, 0x13, 0x97, 0x1f, 0x93, 0xe8, 0x6b, 0x7e, 0xdf, 0x05,
                                                 0x70, 0x54, 0x02};
            ...
            void Header::setAsymKey(jcp::AsymKey *key, bool encMode) {
#if defined(HAS_MBEDTLS) && HAS_MBEDTLS
                ...
                if (key) {
                    std::string algorithm = key->getAlgorithm();
                    if (algorithm == "RSA") {
            ...
            HeaderCipherAlgorithm Header::findHeaderCipherAlgorithm(unsigned char value) {
                static const HeaderCipherAlgorithm *list[] = {&HeaderCipherAlgorithm::NONE,
                                                              &HeaderCipherAlgorithm::V1_RSA,
                                                              &HeaderCipherAlgorithm::EC, &HeaderCipherAlgorithm::RSA,
                                                              NULL};
            ...
            DataCipherAlgorithm Header::findDataCipherAlgorithm(unsigned char value) {
                static const DataCipherAlgorithm *list[] = {&DataCipherAlgorithm::NONE, &DataCipherAlgorithm::AES,
                                                            NULL};
            ...
            int Header::readBuffer(Buffer &buffer, exception::SignedSecureFileException *exception) {
                if (!rawHeaderReaded) {
                    ...
                    if (!_sysIsLitleEndian) {
                        ...
                        int16toArray((unsigned char *) &bufHeader->signedSecureHeaderSize,
                                     bufHeader->signedSecureHeaderSize);
                ...
                if (rawHeaderReaded) {
                    if (buffer.readRemaining() > rawHeader.signedSecureHeaderSize) {
                        ...
                        unsigned char *signedSecureHeader = (unsigned char *) getMemoryAllocator()->allocate(
                                rawHeader.signedSecureHeaderSize);
                        ...
                        do {
                            if (this->headerCipherAlgorithm == HeaderCipherAlgorithm::EC) {
                                uint16_t ecKeySize = (((uint16_t) signedSecureHeader[0]) << 0) |
                                                     ((uint16_t) signedSecureHeader[1]) << 8;
                                ...
                                decodedSecureHeader = (unsigned char *) getMemoryAllocator()->allocate(
                                        rawHeader.signedSecureHeaderSize);
                            ...
                            if (memcmp(secureHeader.sig, SIGNATURE, sizeof(SIGNATURE)) ||
                                secureHeader.sig[15] != rawHeader.version) {
            ...
            bool Header::writeTo(Buffer &buffer, size_t computedHeaderSize,
                ...
                if (signedSecureHeaderBuffer.readRemaining() > 0) {
                    buffer.insert(Header::COMMON_HEADER_SIZE, (const unsigned char *) signedSecureHeaderBuffer.buffer(),
                                  signedSecureHeaderBuffer.readRemaining());
                }
            ...
            void Header::arrayToInt32(const unsigned char *buffer, uint32_t *value) {
                if (_sysIsLitleEndian)
                    ...
                    *value =
                            (((uint32_t) buffer[0]) << 0) | ((uint32_t) buffer[1]) << 8 | ((uint32_t) buffer[2]) << 16 |
